...
class CartpoleAgent:
    ...
    def __init__(self, l_r=0.1, d_r=0.95, r_m=None):
        ...
        self.algorithm = qtable.Qtable(CartpoleAgent.QTABLE_SHAPE, 
                                       "Cartpole", 
                                       l_r=l_r, 
                                       d_r=d_r)
    ...
    def step(self, action):
        s_before_action_disc = CartpoleAgent.get_discrete_state(self.state)
        n_s, r, terminated, truncated, info = self.env.step(action)
        self.state = n_s
        # Update info required for retraining of Q table at the end
        n_s_disc = CartpoleAgent.get_discrete_state(n_s)
    ...
    def get_optimal_action(self, state):
        s_disc = CartpoleAgent.get_discrete_state(state)
    ...
    def get_discrete_state(state):
        ...
        discrete_state = state/CartpoleAgent.NP_ARRAY_WIN_SIZE + np.array([15,10,1,10])
