...
def save_table(table, name=None):
...
for episode in range(EPISODES + 1): #go through the episodes
    ...
    s, _ = env.reset()
    discrete_state = get_discrete_state(s) #get the discrete start for the restarted environment 
    done = False
    ...
    while not done: 
        ...
        new_state, reward, terminated, truncated, _ = env.step(action) #step action to get new states, reward, and the "done" status.
